...
void themSachXtruocSachnhohon50k(LIST &l)
{
	SACH x;
	NODE *p = KhoiTaoNODE(x);
	printf("\nNhap Thong Tin Cuon Sach Can Them");
	cin.ignore();
	printf("\nNhap vao Ma Sach: ");
	scanf("%s",&p->data.MaSach);
	fflush(stdin);
	printf("\nNhap vao Ten Sach: ");
	scanf("%s",&p->data.TenSach);
	fflush(stdin);
	cout << "\nNhap vao So Trang: ";
	cin >>p->data.SoTrang;
	printf("Gia Ban: ");
	cin >> p->data.GiaBan;
	printf("Nam Xuat Ban: ");
	cin >> p->data.NamXuatBan;
	if(l.pHead->data.GiaBan < 50.000 && l.pHead->pNext ==NULL)
	{
		ThemVaoDau(l,p);
	}
	else
	{
		NODE *g = new NODE; // node trung gian giu lien ket
		for(NODE *k = l.pHead ; k!= NULL ; k=k->pNext)
		{
			if(k->data.GiaBan < 50.000)
			{
				NODE *h = KhoiTaoNODE(p->data); // address moi node khac nhau nen phai tao them address cua data cu~
				h->pNext = k;
				g->pNext = h;
			}
			g = k; // node g nam truoc de lien ket 
		}
	}
...
int maint()
{
    ...
    do
    {
        ...
        printf("\t\t\t |5 Them Cuon Sach X truoc cuon sach nho hon 50k|\n");
        printf("\t\t\t |6. Thoat                                      |\n");
        printf("\t\t\t ================================================\n");
        printf("Ban chon: ");
        scanf("%d", &chon);
